...
fs.writeFileSync(path.join(test, 'AndroidManifest.xml'), `<manifest xmlns:android="http://schemas.android.com/apk/res/android" package="com.arman.tvremote.smoketest"><uses-sdk android:minSdkVersion="26" android:targetSdkVersion="36"/><application android:label="Universal TV Remote Tests"/><instrumentation android:name="com.arman.tvremote.SmokeInstrumentation" android:targetPackage="com.arman.tvremote"/></manifest>`);
...
run('javac', ['--release', '17', '-classpath', [androidJar, path.join(build, 'classes')].join(path.delimiter), '-d', path.join(test, 'classes'), 'android/app/src/sdkTest/java/com/arman/tvremote/SmokeInstrumentation.java']);
...
run(path.join(bt, 'apksigner'), ['sign', '--ks', path.join(root, 'android', props.storeFile), '--ks-key-alias', props.keyAlias, '--ks-pass', 'env:TV_APK_STORE_PASS', '--key-pass', 'env:TV_APK_KEY_PASS', '--out', apk, aligned], { env: { ...process.env, TV_APK_STORE_PASS: props.storePassword, TV_APK_KEY_PASS: props.keyPassword } });
const adb = path.join(sdk, 'platform-tools/adb');
run(adb, ['-s', serial, 'install', '-r', 'outputs/Universal-TV-Remote-2.0.0.apk']);
run(adb, ['-s', serial, 'install', '-r', apk]);
const result = run(adb, ['-s', serial, 'shell', 'am', 'instrument', '-w', ...(process.argv.includes('--protocol-mocks') ? ['-e', 'protocolMocks', 'true'] : []), 'com.arman.tvremote.smoketest/com.arman.tvremote.SmokeInstrumentation'], { encoding: 'utf8', stdio: 'pipe' });
